...
class KnowledgeGraphBuilder:
    ...
    def _calculate_edge_weight(self, node1: str, node2: str, type1: str, type2: str) -> float:
        """
        ...
        self.cooccurrence_counts[edge_key] = self.cooccurrence_counts.get(edge_key, 0) + 1
        
        # Adjust weight based on co-occurrence frequency
        frequency_multiplier = min(1 + (self.cooccurrence_counts[edge_key] - 1) * 0.2, 2.0)
    ...
    def visualize(self, figsize=(15, 10)):
        ...
        if edge_weights:  # If there are edges
            ...
            if max_weight == min_weight:
                normalized_weights = [2.5] * len(edge_weights)  # Use constant middle value
            else:
                try:
                    normalized_weights = [(w - min_weight) / (max_weight - min_weight) * 4 + 1 
                                        for w in edge_weights]
                except ZeroDivisionError:
                    normalized_weights = [2.5] * len(edge_weights)  # Fallback to constant width
        else:
            # No edges case
            normalized_weights = []
            max_weight = 0
        ...
        if edge_weights:
            if max_weight != min_weight:
                legend_elements.extend([
                    plt.Line2D([0], [0], color='#CCCCCC', linewidth=1, label='Weak Connection'),
                    plt.Line2D([0], [0], color='#CCCCCC', linewidth=3, label='Medium Connection'),
                    plt.Line2D([0], [0], color='#CCCCCC', linewidth=5, label='Strong Connection')
                ])
            else:
                legend_elements.append(
                    plt.Line2D([0], [0], color='#CCCCCC', linewidth=2.5, label='Uniform Connection')
                )
        ...
        if self.domain_name:
            plt.savefig(f'generated_datasets/{self.domain_name}_.png', bbox_inches='tight')
        plt.show()